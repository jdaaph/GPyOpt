...
def plot_acquisition_binaryT(bounds,input_dim,model,Xdata,Ydata,acquisition_function,suggested_sample, filename = None):
    ...
    if input_dim ==2:
        ...
        plt.plot(Xdata[:,0], Xdata[:,1], 'r.', markersize=10, label=u'Observations')
        plt.contourf(X1, X2, np.sqrt(v.reshape(200,200)),100, cmap='plasma')
        ...
        plt.plot(suggested_sample[:,0],suggested_sample[:,1],'b*', markersize=15)
        plt.contourf(X1, X2, acqu_normalized,100, clim=(0,1), cmap='plasma')
